...
import { GraphContainer, PieGraphRow, PriceGraph } from 'components/Graphs'
import AreaGraph from 'components/Graphs/AreaGraph/AreaGraph'
import DonutGraph from 'components/Graphs/PieGraph'
import { Icon, QuestionIcon } from 'components/Icon'
import { SpinnerLoader } from 'components/Loader'
import Page from 'components/Page'
import { Progress } from 'components/ProgressBar'
import Spacer from 'components/Spacer'
import { StatCard, StatsRow } from 'components/Stats/Stats'
			...
			<Page>
				<Container>
					<NestWrapper>
						<CornerButtons>
							<Tooltipped
								content={`${analyticsOpen ? 'Hide' : 'View'} Analytics`}
							>
								<CornerButton
						...
						<StatsRow lg={4} sm={2}>
							...
							<Col>
								<StatCard>
									...
									<StyledBadge>
										{(supply &&
											`${getDisplayBalance(supply)} ${nestToken}`) || (
											<SpinnerLoader />
										)}
									</StyledBadge>
								</StatCard>
						...
						<Container>
							<NestButtons>
								...
								<Spacer size={'md'} />
								<Button
									href={`https://app.sushi.com/swap?tokens=${nestTokenAddress}&tokens=${Config.addressMap.WETH}&chainId=137`}
									target="_blank"
									text="Swap"
									disabled={!account}
								/>
							...
							<NestAnalytics in={analyticsOpen}>
								<NestAnalyticsContainer>
									<Spacer size={'lg'} />
									{priceHistory &&
										priceHistory[priceHistory.length - 1].close > 0 && (
											<PriceGraph>
												...
												<PrefButtons>
													{_.map(['W', 'M', 'Y'], (timeFrame) => (
														<BootButton
															...
															onClick={() =>
																setPriceHistoryTimeFrame(timeFrame)
															}
