...
const useHomeAnalytics = () => {
	...
	// 			nest.outputToken
	// 		const nestContract = new bao.web3.eth.Contract(experipieAbi as AbiItem[], nestAddress)
	...
	// 	const results = MultiCall.parseCallResults(await multicall.call(MultiCall.createCallContext(multicallContext)))
	// 	let totalNestUsd = new BigNumber(0)
	// 	for (const nestAddress of Object.keys(results)) {
	// 		const _price = reservesPrices[nestAddress.toLowerCase()] || new BigNumber(0)
	// 		const _supply = getBalanceNumber(new BigNumber(results[nestAddress][1].values[0].hex), results[nestAddress][0].values[0])
	...
	// 		{
	// 			title: 'Polly Burned 🔥',
	// 			data: truncateNumber(new BigNumber((await GraphUtil.getPollyBurned()).burnedTokens)),
	// 		},
	// 	])
